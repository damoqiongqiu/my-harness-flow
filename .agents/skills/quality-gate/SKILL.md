...
| 验证层 | 脚本路径 | 不存在时的行为 |
|--------|---------|--------------|
| L1 冒烟 | `quality-gate/l1-smoke/health-check.sh` | 🟡 告知用户「L1 脚本未实现」，**跳过该层但不计 FAIL** |
| L2 单元 | `quality-gate/l2-integration/run-l2-integration.sh` | 🟡 告知用户「L2 脚本未实现」，**跳过该层但不计 FAIL** |
| L3 E2E | `quality-gate/l3-e2e/run-l3-e2e.sh` | 🟡 告知用户「L3 脚本未实现或运行环境不可用」，**跳过** |
| L5 回归 | `quality-gate/l5-regression/run-l5-regression.sh` | 🟡 告知用户「L5 脚本未实现」，**跳过** |
...
bash quality-gate/l3-e2e/run-l3-e2e.sh  # 无运行环境自动跳过
```
