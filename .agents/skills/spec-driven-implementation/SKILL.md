...
在执行任何步骤前，**先检查 `specs/README.md` 是否存在**。若存在，读取它以了解本项目 spec 目录结构、命名规则、各档位 spec 要求以及生命周期规范。

> ⚠️ `specs/README.md` 中的约定**优先于**本 skill 的通用规则。
...
- `write-product-spec`
- `write-tech-spec`
- `start-task`（实现入口，替代上游的 `implement-specs`）
