...
> 3. 复制 `templates/specs/_template-product.md` → `specs/<slug>/product.md`
> 4. **调用 `write-product-spec` SKILL** 填充内容
> 5. 然后继续 §4 创建分支
>
...
3. **从模板实例化两份 spec 骨架文件**：
   - 复制 `templates/specs/_template-product.md` → `specs/<slug>/product.md`
   - 复制 `templates/specs/_template-tech.md` → `specs/<slug>/tech.md`
4. **填充 spec 内容**：
   - **首先调用 `write-product-spec` SKILL**，填充 `specs/<slug>/product.md`。该 skill 会读取现有骨架，按产品 spec 规范完成内容书写
   - **判断是否需要技术 spec**：Standard 或 Full 档任务、架构有显著变化、跨模块/跨服务变更时，**再调用 `write-tech-spec` SKILL** 填充 `specs/<slug>/tech.md`；纯 UI 变更或简单逻辑可省略
...
> ⚠️ 模板分布在目标项目的 `templates/specs/` 下（由 `my-harness-cli.sh` 安装时同步）。若目标项目无此目录，先检查是否为框架本体仓库（则直接使用），否则报错提示用户安装模板。
