...
description: 使用临时目录快照和与 CI 相同的 review.json 合约，从当前分支本地运行仓库 spec 审查工作流。
---

# review-spec-local

在本地 spec 工作完成后、推送或创建 spec PR 前使用此 SKILL。它准备与 GitHub 审查 workflow 相同的审查输入，然后将审查逻辑委托给 `review-spec`。
...
- 如果审查发现问题，通过 `review.json` 报告；在此 SKILL 期间不修复 spec。
